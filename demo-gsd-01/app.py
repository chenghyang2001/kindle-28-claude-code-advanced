"""
待辦清單 CLI — GSD 練習用的極簡 in-memory 待辦管理模組。

這是「已存在的舊 codebase」，提供四個核心函式：
  add_task / list_tasks / delete_task / complete_task
學習者需要在此基礎上以 GSD 流程新增第 4 個功能。
"""
...
def add_task(tasks: list, name: str) -> None:
    ...
    Raises:
        ValueError: 若 name 為空字串，拒絕新增以避免無意義任務。
    """
...
def complete_task(tasks: list, name: str) -> bool:
    ...
    for task in tasks:
        if task["name"] == name:
            # 已完成的任務再次標記是 no-op，冪等設計讓呼叫端不需要先查狀態
            task["done"] = True
            return True
